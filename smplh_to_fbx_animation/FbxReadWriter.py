...
class FbxReadWrite(object):
    ...
    def addAnimation(self, pkl_filename:str, smpl_params:Dict, verbose:bool = False):
        ...
        for idx, name in enumerate(names):
            ...
            rotvec = smpl_poses[:, idx*3:idx*3+3] #[n, 24*3]
            if rotvec.shape[1] == 0:
                rotvec = np.zeros((n,3))
            _euler = []
        ...
        # Translation on the Y axis (in blender, this is not the vertical axis 
        # but one of the axis that forms the horizontal plane)
    ...
    def writeFbx(self, write_base:str, filename:str):
        ...
        write_path = os.path.join(write_base, "fbx_animation.fbx")
